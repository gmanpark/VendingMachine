...
jindo.$Jindo.checkVarType._throwException = function(aArgs, oRules, sFuncName) {
    ...
    if (/(\$\w+)#(\w+)?/.test(sFuncName)) {
        sURL = 'http://jindo.nhncorp.com/docs/jindo/archive/Jindo2-2.9.0/desktop/ko/classes/' + encodeURIComponent(RegExp.$1) + '.html' + "#method_"+RegExp.$2;
    }
...
결과 캐시를 사용중이면 캐시에 저장
 ...
 @lends $$
		...
		cssquery = function(sQuery, oParent, oOptions){
			...
			sTempId, aRet, nParentNodeType, bUseQueryId, oOldParent, queryid, _clone, sTagName;
        ...
        if(document.documentMode == 9){
            jindo.$Element._eventBind = function(oEle,sEvent,fAroundFunc,bUseCapture){
                if(/resize/.test(sEvent) ){
                    oEle._attachEvent("on"+sEvent,fAroundFunc);
                }else{
