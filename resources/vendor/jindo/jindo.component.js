/** Jindo Component(minify:0) : ../../../docs/jindo-component/archive/1.11.0/
 *	jindo.Component
 *	jindo.Timer
 *	jindo.Effect
 *	jindo.Transition
...
/**
	...
	@since 1.3.0
 */
jindo.Component.VERSION = '1.11.0'; /**
...
var G_vmlCanvasManager_={init:function(opt_doc){if(/MSIE/.test(navigator.userAgent)&&!window.opera){var doc=opt_doc||document;doc.createElement('canvas');doc._attachEvent('onreadystatechange',bind(this.init_,this,doc));}},init_:function(doc){if(!doc.namespaces['g_vml_']){doc.namespaces.add('g_vml_','urn:schemas-microsoft-com:vml','#default#VML');}
...
var els=doc.getElementsByTagName('canvas');for(var i=0;i<els.length;i++){this.initElement(els[i]);}},initElement:function(el){if(!el.getContext){el.getContext=getContext;el.innerHTML='';el._attachEvent('onpropertychange',onPropertyChange);el._attachEvent('onresize',onResize);var attrs=el.attributes;if(attrs.width&&attrs.width.specified){el.style.width=attrs.width.nodeValue+'px';}else{el.width=el.clientWidth;}
...
jindo.Keyframe = jindo.$Class({
	...
	**/
	...
	/**
		...
		@chainable
	**/
	frame : function(nRate) {

		if (nRate === this._nCurRate) { return; }

		var bHasTransformRenderBug = this._bHasTransformRenderBug;

		var aLists = this._get(nRate, false);
		...
		for (var i = 0, nLen = aLists.length; i < nLen; i += 2) {
			...
			for (var sKey in oProps) if (oProps.hasOwnProperty(sKey)) {
				...
				if (/^@(.*)$/.test(sKey)) {
					...
					try {
						welObj.css(this._getCSSKey(sKey), vProp);
					} catch(e) {
...
/**
	...
	@history 1.7.0 bug bUseTranstion 옵션이 true 인 경우 발생하는 pause 메소드 버그 수정
	@history 1.7.0 bug bUseTranstion 옵션이 true 인 경우, 시작점이 반영되지 않는 버그 수정
	@history 1.7.0 bug display CSS 속성이 none 인 엘리먼트를 CSS Transition으로 변환 할 때 뒤이어 애니메이션이 진행되지 않는 버그 수정
**/
jindo.Morph = jindo.$Class({
	...
	**/
	 ...
	 */
	_processItem : function(nRate, bSetStatic, nTargetType, bPause, DEBUG) {
	...
	**/
	pause : function(nRate) {
		...
		var aNormalPropsToPause = this._processItem(nRate, true, 3/*ALL*/, true, 'DEBUG').normalPropsToPause;
...
jindo.SelectBox = jindo.$Class({
    ...
    **/
    _paintList : function() {
        ...
        for (var j = 0, len = aChildLIs.length; j < len; j++) {
            var elItem = aChildLIs[j];
            this._aListItem.push(elItem);
            this._aItemData[aTempKey[j]].elItem = elItem;
        }
...
jindo.StarRating = jindo.$Class({
	...
	**/
	setValue : function(nValue, bFireEvent) {
		...
		if (bFireEvent) {
			...
			**/
			this.fireEvent("set", { nValue : this._nDropMoney });
		}
	...
	**/
	...
	_onMouseLeave : function(we) {
		this.setValue(this._nDropMoney, false);
		/**
			마우스가 별점 위에서 벗어났을 때
