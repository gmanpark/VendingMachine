...
nts.Component.DragArea = jindo.$Class({
    ...
    _cachedFunction: function () {
        this._fnOnLeftMouseButton = function (we) {
            return we._event.which === 1 // LeftButton : 1 ScrollButton : 2 RightButton : 3
        };
    ...
    _onDragStart: function (we) {
        ...
        this.fireEvent('onDrag',  we.srcElement); // fire Custom Event
    },

    _onDrag: function (we) {
        if (this._fnOnLeftMouseButton(we) && this._bIsDragging) {
            this._welDraggableArea.after(this._welCopyDraggableArea);
            var nTopPosition = we.pos().pageY - (parseInt(this._welCopyDraggableArea.css("height"), 10) / 2);
            var nLeftPosition = we.pos().pageX - (parseInt(this._welCopyDraggableArea.css("width"), 10) / 2);
