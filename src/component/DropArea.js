...
nts.Component.DropArea = jindo.$Class({
    ...
    _onDrop: function (we) {
        ...
        if (this._isConflict(htMousePos) && this._isOverDragElement(we)) {
            this.fireEvent('onDrop', we.srcElement);
        }
    },

    _isConflict: function (htMousePos) {
        var nLeftDropArea = this._welDroppableArea.offset().left;
        var nTopDropArea = this._welDroppableArea.offset().top;
        var nRightDropArea = nLeftDropArea + parseInt(this._welDroppableArea.css("width"), 10);
        var nBottomDropArea = nTopDropArea + parseInt(this._welDroppableArea.css("height"), 10);
